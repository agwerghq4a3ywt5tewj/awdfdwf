...
public class TempestDragonBoss extends GodBoss {
    ...
    protected void setupBoss() {
        if (entity instanceof EnderDragon dragon) {
            // Set proper health and damage
            if (entity.getAttribute(Attribute.MAX_HEALTH) != null) {
                entity.getAttribute(Attribute.MAX_HEALTH).setBaseValue(bossType.getMaxHealth() * healthMultiplier);
                entity.setHealth(bossType.getMaxHealth() * healthMultiplier);
            }
            if (entity.getAttribute(Attribute.ATTACK_DAMAGE) != null) {
                entity.getAttribute(Attribute.ATTACK_DAMAGE).setBaseValue(bossType.getDamage() * damageMultiplier);
            }
            ...
            dragon.setCustomName("§e§lStorm Sovereign");
                    ...
                    plugin.getServer().getScheduler().runTaskLater(plugin, () -> {
                        ...
                        for (Player nearbyPlayer : entity.getWorld().getPlayers()) {
                            if (nearbyPlayer.getLocation().distance(strikeLoc) <= 5) {
                                nearbyPlayer.damage(getScaledDamage(15.0));
                                nearbyPlayer.addPotionEffect(new PotionEffect(PotionEffectType.GLOWING, 
                                    getScaledEffectDuration(100), getScaledEffectLevel(0)));
    ...
    private void windShear() {
        ...
        for (Player player : entity.getWorld().getPlayers()) {
            if (player.getLocation().distance(entity.getLocation()) <= getScaledRadius(25.0)) {
                ...
                player.damage(getScaledDamage(10.0));
                player.addPotionEffect(new PotionEffect(PotionEffectType.LEVITATION, 
                    getScaledEffectDuration(60), getScaledEffectLevel(1)));
                ...
                for (Player player : entity.getWorld().getPlayers()) {
                    if (player.getLocation().distance(tornadoCenter) <= 8) {
                        ...
                        if (player.getLocation().distance(tornadoCenter) <= 3) {
                            player.damage(getScaledDamage(5.0));
                            player.addPotionEffect(new PotionEffect(PotionEffectType.NAUSEA, 
                                getScaledEffectDuration(100), getScaledEffectLevel(1)));
                        }
                    }
                }
    ...
    protected void onDeath() {
        ...
        // Drop Tempest God Fragment 7 (final fragment)
        loc.getWorld().dropItemNaturally(loc, com.fallengod.testament.items.FragmentItem.createFragment(GodType.TEMPEST, 7));
        
        // Announce death
        for (Player player : entity.getWorld().getPlayers()) {
            if (player.getLocation().distance(loc) <= 200) {
                player.sendTitle("§e§lSTORM SOVEREIGN DEFEATED", "§7The tempest has been calmed", 20, 80, 20);
                player.sendMessage("§6§l⚔ The Storm Sovereign has been vanquished! The skies are calm once more! ⚔");
            }
        }
    }
}